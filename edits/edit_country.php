    ...
    <div class="container col-8 my-4">
        ...
        <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" class="text-center" method="post" enctype="multipart/form-data">
            <div class="form-group">
                <h2 class='text-center my-2'>Изменить страну</h2>
            </div>

            <div class="form-group">
                <input type="text" class="form-control" name="country_name" placeholder="Название страны" value="<?php echo $country->name; ?>">
            </div>
                        ...
                        <?php 
                            if(isset($regions)){
                                foreach($regions as $region){ ?>
                                    <option <?php if($_GET['region_id'] != ""){ if($region['id'] == $_GET['region_id']){ echo 'selected'; } } elseif($region['id'] == $country->region_id){ echo 'selected'; } ?> value="<?php echo $region['id'] ?>"><?php echo $region['name']; ?></option>
                                <?php }
                            }
                        ?>
                    </select>
            ...
            <div class="form-group">
                ...
                <button class="btn btn-lg btn-success w-50" type="submit" name="edit_country">Изменить страну</button>
            </div>
        </form>
    </div>
